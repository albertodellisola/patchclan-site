...
export const MONSTERMAKER = {
  ...
  sistema: 'Famicom', mapper: 'MMC1 · 256 KB PRG · CHR-RAM de 8 KB · save com bateria',
  categoria: bi('Tradução', 'Translation', 'Traducción'),
  nivel: 'alfa',
  capa: 'mm/01-titulo.png',
  linha: bi('Um jogo de cartas japonês virou RPG de Famicom. A fonte do cartucho tem três letras latinas: H, M e P. O resto será desenhado.',
            'A Japanese card game became a Famicom RPG. The cartridge font holds three Latin letters: H, M and P. The rest gets drawn.',
            'Un juego de cartas japonés se volvió RPG de Famicom. La fuente del cartucho tiene tres letras latinas: H, M y P. El resto se dibuja.'),
  resumo: {
    pt: ['<i>Monster Maker</i> nasceu como jogo de cartas japonês — monstro, tesouro e masmorra distribuídos carta a carta — e foi popular o bastante para virar RPG de Famicom em <i>7 Tsu no Hihou</i>, os sete tesouros. Fora do Japão o nome não diz nada a ninguém, e é exatamente esse o tipo de cartucho que some da história por não ter idioma, não por não prestar. Ele salva em bateria e abre com um prólogo que começa em <code>はるかなる むかし</code>, num tempo muito distante, antes de perguntar se você começa do zero ou retoma um dos dois livros do tempo.',
          'O reconhecimento está fechado, e fechado <b>rodando a ROM intocada</b>, não deduzido no papel: o percurso foi feito no emulador até a primeira cena, e o encoding está provado por trace — o índice do tile <b>é</b> o código do caractere, o bit alto do byte acrescenta um espaço depois da letra, <code>$FF</code> encerra a mensagem, <code>$BB</code> troca de página, <code>$BC</code> quebra a linha e <code>$7F</code> seguido de dois bytes puxa uma palavra do dicionário. Foi esse bit alto que fazia a busca ingênua por uma frase falhar. Com o motor entendido, o roteiro saiu inteiro: <b>1.386 mensagens</b>, <b>33.564 bytes</b>, cerca de <b>44.750 caracteres</b>.',
          'O obstáculo já tem nome e tamanho. A fonte tem <b>512 tiles</b> e exatamente <b>três letras latinas</b> — <b>H</b>, <b>M</b> e <b>P</b>, as de HP e MP —, de modo que traduzir este jogo começa por desenhar o alfabeto inteiro, letra por letra, sobre as <b>118 casas de kana livres</b>, antes de a primeira frase existir. E o espaço é duro: a ROM inteira guarda <b>512 bytes livres, 0,2% dela</b>, para um inglês que vai pedir de <b>85 a 100 KB</b>. Por isso a expansão para <b>512 KB</b> é a peça central, e ela já está <b>provada</b>: a ROM grande roda idêntica à original, e o A18 — o bit 4 do registrador de banco de CHR, ocioso porque o CHR é RAM — alcança o segundo bloco. Não há patch para instalar; esta página é o registro de onde o trabalho está.'],
    en: ['<i>Monster Maker</i> began as a Japanese card game — monster, treasure and dungeon dealt card by card — and was popular enough to become a Famicom RPG in <i>7 Tsu no Hihou</i>, the seven treasures. Outside Japan the name means nothing to anyone, and that is exactly the sort of cartridge that vanishes from history for having no language rather than for being no good. It saves to battery and opens on a prologue beginning <code>はるかなる むかし</code>, in a time long past, before asking whether you start from scratch or resume one of the two books of time.',
          'The survey is closed, and closed <b>by running the untouched ROM</b>, not inferred on paper: the route was walked in the emulator up to the first scene, and the encoding is proved by trace — the tile index <b>is</b> the character code, the high bit of the byte adds a space after the letter, <code>$FF</code> ends the message, <code>$BB</code> turns the page, <code>$BC</code> breaks the line, and <code>$7F</code> followed by two bytes pulls a word from the dictionary. It was that high bit that made a naive search for a sentence fail. With the engine understood, the script came out whole: <b>1,386 messages</b>, <b>33,564 bytes</b>, roughly <b>44,750 characters</b>.',
          'The obstacle already has a name and a size. The font holds <b>512 tiles</b> and exactly <b>three Latin letters</b> — <b>H</b>, <b>M</b> and <b>P</b>, the ones from HP and MP — so translating this game starts by drawing the whole alphabet, letter by letter, over the <b>118 free kana slots</b>, before a single sentence can exist. And space is unforgiving: the entire ROM holds <b>512 free bytes, 0.2% of it</b>, for an English that will ask for <b>85 to 100 KB</b>. That makes the expansion to <b>512 KB</b> the centrepiece, and it is already <b>proved</b>: the large ROM runs identically to the original, and A18 — bit 4 of the CHR bank register, idle because the CHR is RAM — reaches the second block. There is no patch to install; this page records where the work stands.'],
    es: ['<i>Monster Maker</i> nació como juego de cartas japonés — monstruo, tesoro y mazmorra repartidos carta a carta — y fue popular lo bastante para convertirse en RPG de Famicom en <i>7 Tsu no Hihou</i>, los siete tesoros. Fuera de Japón el nombre no le dice nada a nadie, y ese es exactamente el tipo de cartucho que desaparece de la historia por no tener idioma, no por no valer. El cartucho guarda con pila y abre con un prólogo que empieza en <code>はるかなる むかし</code>, en un tiempo muy lejano, antes de preguntar si empiezas de cero o retomas uno de los dos libros del tiempo.',
          'El reconocimiento está cerrado, y cerrado <b>corriendo la ROM intacta</b>, no deducido sobre el papel: el recorrido se hizo en el emulador hasta la primera escena, y la codificación está probada por traza — el índice del tile <b>es</b> el código del carácter, el bit alto del byte añade un espacio detrás de la letra, <code>$FF</code> cierra el mensaje, <code>$BB</code> cambia de página, <code>$BC</code> rompe la línea y <code>$7F</code> seguido de dos bytes saca una palabra del diccionario. Fue ese bit alto el que hacía fallar la búsqueda ingenua de una frase. Con el motor entendido, el guion salió entero: <b>1.386 mensajes</b>, <b>33.564 bytes</b>, unos <b>44.750 caracteres</b>.',
          'El obstáculo ya tiene nombre y tamaño. La fuente tiene <b>512 tiles</b> y exactamente <b>tres letras latinas</b> — <b>H</b>, <b>M</b> y <b>P</b>, las de HP y MP —, de modo que traducir este juego empieza por dibujar el alfabeto entero, letra a letra, sobre las <b>118 casillas de kana libres</b>, antes de que exista la primera frase. Y el espacio es duro: la ROM entera guarda <b>512 bytes libres, el 0,2% de ella</b>, para un inglés que va a pedir de <b>85 a 100 KB</b>. Por eso la ampliación a <b>512 KB</b> es la pieza central, y ya está <b>probada</b>: la ROM grande corre idéntica a la original, y el A18 — el bit 4 del registro de banco de CHR, ocioso porque el CHR es RAM — alcanza el segundo bloque. No hay parche que instalar; esta página es el registro de dónde está el trabajo.']
  },
  numeros: [
    { v: '1.386', r: bi('mensagens no roteiro', 'messages in the script', 'mensajes en el guion') },
    { v: '3', r: bi('letras latinas na fonte', 'Latin letters in the font', 'letras latinas en la fuente') },
    { v: '512', r: bi('bytes livres na ROM inteira', 'free bytes in the whole ROM', 'bytes libres en toda la ROM') }
  ],
  grupos: [],
